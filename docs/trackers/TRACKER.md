...
| Cession file detect/map/clauses/validate stages | Working | Detect, map-contract, clauses, validate, process-exceptions, process, approve, summary, and pipeline-status endpoints are wired end to end and frontend-connected; Pension Status detection/manual override, DB-derived contract clauses, pandas validation, active-member coverage checks, exception creation, and processing now use the confirmed uploaded CSV or Excel (`.xlsx`) file plus current DB population rows instead of pre-confirmation sample rows |
