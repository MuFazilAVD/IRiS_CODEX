...
| Feature | Status | Notes |
|---------|--------|-------|
| Cession file processing | Partial | Queue page now supports the screenshot-backed full-page upload/history workflow at `/claims/cession-files/new` and `/claims/cession-files/:fileId`; upload stores source content and waits for confirmed detection/mapping before creating validation artifacts; cedent/contract detection now prioritizes strong filename matches, uses OpenAI fallback for low-confidence/conflicted matches when configured, clears downstream records/exceptions when overrides change, Pension Status CSV or Excel (`.xlsx`) files validate against confirmed-contract `policy_register` rows before applying SCD2 population updates, and Settlement files are detected by the required settlement header signature, including tab-delimited CSV aliases such as `Applicable Indexation / Escalation`, `Fee (Admin)`, `Settlement Period`, `Fixed Amount`, and `Floating Amount`, before exact fixed/floating/net reconciliation; Settlement validation now surfaces settlement-only repair suggestions in the Anomalies and Resolutions flow for header remapping, ISO date normalization, mode-based date/text imputation, and currency/amount cleanup, while the Resolutions screen now defaults every fix to `Accept`, shows the renamed top-right `Accept All Fixes` action above the table, and keeps Override/Manual available when needed; the Clauses step now reads from the shared hardcoded contract clause catalog for Settlement uploads and shows only the `Settlement` category rows; the Clauses to Anomalies transition now pre-runs validation so anomaly rows are populated before the Anomalies table is shown, and reopened files that were still parked at the pre-validation validate step auto-hydrate that table on load; accepted or overridden Settlement resolutions update the processed record state used by downstream reconciliation; Settlement processing now aggregates normalized incoming fixed leg, floating leg, fee, interest, and net values across all file rows, uses the pending `settlements` table row for cedent/contract/quarter as ground truth when present, falls back to the same contract-performance quarter values shown on the contract detail financial card before any mock baseline is used, treats admin fee as a settlement deduction when recomputing net, routes mismatches to reconciliation review, and on exact match creates both Settlement Pending and Sanction Screening worklist items with an initial `open`/not-started state; the exact-match path now also creates a new pipeline settlement row in the Settlements register using a cession-file-specific settlement ID while preserving the pending baseline row for future reconciliation lookups, and persists a real `cession_file` sanctions case into the Sanctions workspace; the pipeline Worklist step now exposes both generated worklist IDs as clickable links into `/worklist/:wlId`; exact-match downstream cash tracker/GRDR files are generated into a pipeline `Files` step for view/download before the user pushes them to SFTP/Reports; settlement expectations skip incomplete sample `policy_register` populations and use the documented deterministic mock fallback only after the contract-performance alignment path is exhausted; richer audit presentation and contract-period expectations remain partly mock-backed where schema-tracked data is unavailable. Non-settlement pipeline headers are now explicitly labeled `(beta)` while settlement retains the standard header copy |
