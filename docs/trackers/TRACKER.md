...
| Settlements | Working | Screenshot-backed settlement register now uses the 7-card `Settlement & Reconciliation` summary strip, 3-chart analytics row, compact search/status/list-grid toolbar, latest-updated settlement ordering so newly created cession settlement cases appear first, and the current baseline register is aligned to `Q1 2026` for Northstar/Helvetia/Maple/Bavarian/Atlas so incoming `2026 Q1` settlement uploads compare against the current seeded/live register; export/statement actions, the right-side detail panel, approve/hold/dispute actions, and `audit_events`-backed detail-panel audit trails remain in place |
